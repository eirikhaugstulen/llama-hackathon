...
export default function QuestionInput() {
  const suggestions = [
    ...
    "Dengue vs Malaria in Damongo District Hospital this quarter",
  ]
    ...
    <div className="relative w-full max-w-3xl mx-auto">
      <div className="flex flex-wrap gap-2 mb-3">
        {suggestions.map((suggestion, index) => (
          <Button key={index} variant="outline" size="sm" className="text-xs bg-white hover:bg-gray-100">
            {suggestion}
          </Button>
        ))}
      </div>
      <div className="relative flex items-center gap-2 p-2 bg-white rounded-[20px] shadow-sm border border-gray-100">
        <Input
          type="text"
